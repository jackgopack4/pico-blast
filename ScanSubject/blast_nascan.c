                           ...
                           Int4 max_hits, Int4 * scan_range) 
{
  ...
  const BLAST_SequenceBlk * query = searchQuery; 
  Uint1 *s;
  ...
  Uint1 temporary_database_index;
  Int4 current_database_index;
  Uint1 temporary_result_count;
  Int4 current_offset_pair = 0;
 
  Uint1 database_stream = 1;
  Uint1 results_stream = 2;
 
  // SET BIT FILE NAME HERE
  bitFileName = "ScanSubject_8_4.bit";
  ...
  printf("Opening streams to FPGA\n");
  stream = pico->CreateStream(database_stream);
  if (stream < 0) {
    fprintf(stderr,"couldn't open database stream! (code: %i)\n", stream);
    exit(1);
  }
  ...
  // write query length and subject length to picobus
  if((err = pico->WriteDeviceAbsolute(0x4000, subject->length, 4)) < 0) {
    fprintf(stderr, "Error Writing Subject Length");
    exit(1);
  }
  if((err = pico->WriteDeviceAbsolute(0x4004, query->length, 4)) < 0) {
    fprintf(stderr, "Error Writing Query Length");
    exit(1);
  }
  ...
  while(write_query_count < (query->length) / COMPRESSION) {
    ...
    write_query_count+=FULL_128_BITS;
  }
  ...
  while(current_database_index < s_end) {
    ...
    if(pico->GetBytesAvailable(database_stream, WRITESTREAM) >= FULL_128_BITS) {
      ...
      current_database_index+=(FULL_128_BITS*COMPRESSION);
    }
        
    // If output stream has new data, process it
    if(pico->GetBytesAvailable(results_stream, READSTREAM) >= FULL_128_BITS) {
      // Read 128 bits of stream if they are available
      pico->ReadStream(resultsStream,tempResults, FULL_128_BITS);
      // Determine amount of hits by last 8 bits of returned data
      temporary_result_count   = tempResults[15];
      temporary_database_index = tempResults[14];
      total_hits+=temporary_result_count;
      // iterate through the hits and calculate the index of the 12-bit numbers
      for(j=0;j<temporary_result_count,j++) {
        if(j%2) { 
          // Take bottom four bits of left byte, shift and concatenate with right byte
          current_query_index = ((tempResults[j*2-1] & 0xF) << 8) | tempResults[j*2];
        }
        else {
          // Take left byte and shift left, concatenating with upper four bits of right byte
          current_query_index = ((tempResults[j*2])<< 4) | ((tempResults[j*2+1] & 0xF0) >> 4);
        }
        // Set query offset of current offset pair to query index value and database offset to 
        // database + current query value
        offset_pairs[current_offset_pair] -> q_off = current_query_index;
        offset_pairs[current_offset_pair] -> s_off = current_database_index + 
            temporary_database_index + current_query_index;
        current_offset_pair++;
      }
    }
  }
